...
- 故事梗概必须由你根据一级场景和二级场景随机生成，不使用旧梗概。
...
function buildFallbackReferencePrompt({
  ...
  const base = `参考图${index}《${title}》，用于${context.primaryScene}/${context.secondaryScene}短视频。故事梗概：${storyBrief}。画风：${style}。`;

  if (kind === "character") {
    return appendDetailFocus(
      `${base}请生成核心人物设定图，人物身份、年龄感、服装、发型、神态与故事冲突匹配。${getCharacterReferenceSuffix(context.primaryScene)}`,
  ...
  if (kind === "scene") {
    return appendDetailFocus(
      `${base}请生成可直接用于分镜参考的场景设定图，强调空间层次、主体动线、镜头景别、光线方向、色调和情绪氛围；画面干净，无文字水印，不要出现清晰可读文字。`,
      getDetailFocus(kind, index)
    );
  }
  ...
  if (kind === "prop") {
    return appendDetailFocus(
      `${base}请生成关键道具设定图，道具造型与剧情转折强相关，突出材质、磨损细节、摆放方式、局部特写和可被镜头识别的轮廓；无文字水印，不要出现清晰可读文字。`,
      getDetailFocus(kind, index)
    );
  }
...
function ensureReferenceImagesCount(
  ...
  return normalized.map((img, index) => {
    return {
      ...img,
      index: index + 1,
      prompt: img.prompt.replace(/细分考点：[^。\n]*。?/g, "").trim(),
    };
  });
}
...
function buildReferenceAudioMarkdown(audios: GeneratedReferenceAudio[], context: PromptContext) {
  ...
  return normalizeReferenceAudios(audios, context).map((audio) => {
...
function buildReferenceMaterialHeader(
  ...
  const audioLines = normalizeReferenceAudios(audios, context).map((audio) => {
...
function validateGeneratedPrompt(result: GeneratedPrompt, context: PromptContext) {
  ...
  const promptBody = ensureStrictPromptMarkdown(result.finalPromptMarkdown, images, context);
  const materialHeader = buildReferenceMaterialHeader(images, audios, context);

  return {
    generatedStoryBrief: storyBrief,
    finalPromptMarkdown: `${materialHeader}\n\n${promptBody}`.trim(),
    referenceAudioMarkdown: buildReferenceAudioMarkdown(audios, context),
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ragQuery = [
    ...
    .filter(Boolean)
    .join(" ");
  const ragContext = formatScriptRagResults(searchScriptNarrativeRag(ragQuery, 5));

  const userPrompt = `请根据以下项目信息生成视频 Prompt：

故事梗概：不要使用用户输入梗概；请根据一级场景和二级场景随机生成一个新的短视频故事梗概，并返回到 generatedStoryBrief 字段。
...
本地剧情 RAG 参考（仅用于剧情推进、冲突设计、道具证据、对白节奏和叙事质感；不要照搬原文，不要改变 finalPromptMarkdown 的模板结构，不要输出 RAG 来源、检索说明或数据库字样）：
${ragContext}
...
referenceAudios 数组数量必须等于“音色参考数量 + BGM参考数量”。前者必须是人物、旁白或角色音色，必须写具体作品名和具体角色名，并且必须匹配人物说话语言：中文对白找中文影视/动画角色，日语对白找日本影视/动画角色，英语对白找英语影视/动画角色，其他语言同理。后者必须是 BGM，sourceCharacter 必须填“背景BGM”，sourceWork 必须是现有影视剧/电影/动画配乐或现有音乐曲目，禁止写“当前项目”“自选BGM”“原创BGM”“项目统一配乐”。每条都要根据本次随机故事里的角色功能和本次生成批次ID动态选择具体出处，不要每次固定输出同一批角色。若上一轮参考音频出处不是“无”，本轮必须尽量避开上一轮相同的 sourceWork/sourceCharacter 组合，至少更换主要人物音色出处。如果音色参考数量和 BGM参考数量都为 0，referenceAudios 返回空数组。
...
不要输出元信息块，不要输出【画幅】，不要输出画幅比例，不要输出时间戳，不要输出参考图 Prompt/参考视频描述/参考音频描述等旧章节。参考素材清单只列编号和名称/用途，不要放图片生成 Prompt、音频详细出处或参考视频生成 Prompt。镜头个数必须根据视频时长和剧情复杂度动态生成：10秒以内2-3个，10-20秒3-5个，20-40秒5-7个，40-60秒7-10个，60-90秒10-14个，90秒以上14个以上。1-15秒视频 finalPromptMarkdown 约900个中文字符，16-30秒视频约1400个中文字符，上下浮动约15%。每个镜头都必须按“景别/运镜/内容/背景音乐/转场”字段格式输出，其中背景音乐和转场按需出现。运镜与转场必须合理、有逻辑承接，不能突兀炫技；转场必须说明动作、遮挡、光色、视线、道具、运动方向、声音节奏或情绪推进中的承接依据。参考视频只能在运镜或转场字段中作为（参考视频X）贴近标注，不要生成参考视频 Prompt；参考音频在内容或背景音乐字段中作为（参考音频X）贴近标注，详细出处交给 referenceAudios 字段。`;
  ...
  try {
    const generated =
      textProvider === "openai"
        ? await generateWithOpenAIResponses({ apiKey, model, userPrompt })
    ...
    const result = validateGeneratedPrompt(generated, {
